...
//
// It needs python3 with grpcio+protobuf on PATH and write access to
// /usr/local/bin (the backend execs cr_client at that pinned path), so it
// only runs when CROSS_LANG_TEST=1 — set by the containerized test step,
// where both hold.
...
func TestCrossLanguageMemoryRegions(t *testing.T) {
	...
	t.Cleanup(func() { _ = workload.Process.Kill() })
