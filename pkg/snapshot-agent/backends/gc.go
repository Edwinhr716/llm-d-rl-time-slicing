...
// sweepSnapshotStore drops snapshot slot dirs untouched for longer than
// SNAPSHOT_TTL_HOURS (default 6), so stale slots from finished runs don't
...
func sweepSnapshotStore(ctlDir string, now time.Time) {
	...
	for _, e := range entries {
		...
		if err := os.RemoveAll(filepath.Join(snapDir, e.Name())); err == nil {
			slog.Info("GC: removed expired snapshot slot", "slot", e.Name(), "age", now.Sub(info.ModTime()).Round(time.Minute).String())
		}
	}
}
