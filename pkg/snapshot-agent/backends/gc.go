...
//                     ~27Gi of hugepages even after the process dies)
...
func sweep(ctlDir string) {
	...
	sweepSnapshotStore(ctlDir, now)
}

// sweepSnapshotStore drops snapshot group dirs untouched for longer than
// SNAPSHOT_TTL_HOURS (default 6). Groups live on tmpfs, so stale ones from
// finished runs hold RAM, not disk.
func sweepSnapshotStore(ctlDir string, now time.Time) {
	ttl := 6 * time.Hour
	...
	for _, e := range entries {
		...
		if err := os.RemoveAll(filepath.Join(snapDir, e.Name())); err == nil {
			slog.Info("GC: removed expired snapshot group", "group", e.Name(), "age", now.Sub(info.ModTime()).Round(time.Minute).String())
		}
	}
}
