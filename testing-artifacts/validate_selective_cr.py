"""Smoke test for the memory-regions backend: snapshot one CUDA tensor's
device range, restore it, and verify the bytes (CPU and GPU compare).

Ported to the upstream BackendConfig API: regions travel in
MemoryRegionsBackendConfig (snapshot_name names the slot; group is unused).
"""
...
from timeslice.snapshot_agent import MemoryRegion, SnapshotAgentClient, memory_regions_config
...
def main():
    ...
    try:
        ...
        client.check_health("memory-regions")
    ...
    regions = [MemoryRegion(pid=pid, address=addr, size_bytes=size_bytes)]
    print(f"Region: pid={pid} address={hex(addr)} size={size_bytes}")

    job_id = os.getenv("JOB_ID", "test-job")
    snapshot_name = os.getenv("SNAPSHOT_NAME", "validate-slot")
    backend_config = memory_regions_config(regions, snapshot_name=snapshot_name)

    # 3. Trigger Snapshot
    print("Triggering snapshot...")
    try:
        resp = client.snapshot_and_wait(job_id=job_id, backend_config=backend_config)
    ...
    try:
        resp = client.restore_and_wait(job_id=job_id, backend_config=backend_config)
